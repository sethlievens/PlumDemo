"""
Extends the DELI/DAIRY holdout calibration (python/15_holdout_calibration.py)
to the remaining four families: BREAD/BAKERY, PRODUCE, GROCERY, BEVERAGES.
Same protocol, no shortcuts:
  - WIDE z grid -1.5 .. +2.5 (the DELI optimum was negative; a truncated
    range would miss it). Boundary optima are flagged and must be extended.
  - Fit the multiplier on TRAIN stores (3/13/28), evaluate the corrected
    engine on held-out TEST stores (34/41/44) vs the test stores' own optimum.
  - Report train/test multiplier stability per family (the "is it real" test).

UNIDENTIFIABLE CASE (expected for GROCERY/BEVERAGES): when EXP(-shelf_life/
window) is ~0 (365/270-day shelf life vs a 14-20 day window), the spoilage
term is already ~0 and Co = carrying regardless of the multiplier -- the
multiplier is mathematically unidentifiable (0/0), NOT 1.0-by-measurement.
That is a RESULT: v1's exponential was already right for long shelf life.

TWO HARNESS ROBUSTNESS FIXES (both harness, not engine):
  - No @family_filter: passing it as a pyodbc param beside a literal in the
    same EXEC silently drops the filter. Derive ALL families each z
    (deterministic 2172 rows), read back only the target family's keys. Also
    lets one derive per z serve all four families.
  - Fresh connection + retry/reconnect: a single pyodbc connection goes stale
    across the ~7-min pure-numpy calibration phase and the server drops it
    mid-loop. Reconnect right before the derive loop and retry each derive.

Production: identifiable families get their FULL-DATA multiplier;
unidentifiable families are pinned at 1.0. DELI/DAIRY untouched. Then
scenarios 3/4/5 are re-derived.
"""
...
FAMILIES = ["BREAD/BAKERY", "PRODUCE", "GROCERY", "BEVERAGES"]
Z_GRID = [round(-1.5 + 0.25 * i, 2) for i in range(17)]  # -1.5 .. +2.5
EXP_IDENTIFIABLE_MIN = 0.01

DB = {}  # {'conn','cur'} -- rebuilt by connect() on drop
...
def derive_all_and_read(override_z):
    """Derive ALL families into the scratch scenario at override_z (None =
    engine-derived), return {(store_nbr, item_nbr): par_units}. Retries with a
    fresh connection on a dropped/terminated connection."""
    def once():
...
def main():
    ...
    connect()  # fresh connection for the derive loop (the one above went idle through calibrate/run_baseline)

    print(f"\n=== sweep z in [{Z_GRID[0]}, {Z_GRID[-1]}] ({len(Z_GRID)} points), all four families ===")
    for z in Z_GRID:
    ...
    for family in FAMILIES:
        ...
        exp_term = float(np.exp(-S / W))
        identifiable = exp_term >= EXP_IDENTIFIABLE_MIN
        ...
        if identifiable:
            ...
            k_train = k_test = k_full = 1.0
        report[family] = dict(exp_term=exp_term, identifiable=identifiable,
                              zstar_train=zstar_tr, zstar_test=zstar_te, zstar_full=zstar_fu,
                              boundary=not (tr_int and te_int and fu_int),
                              k_train=k_train, k_test=k_test, k_full=k_full, te_min_total=float(te.min()))

    print("\n=== held-out engine: train-fit multipliers, evaluate on test stores ===")
    for family in FAMILIES:
    ...
    for family, r in report.items():
        if r["identifiable"]:
            set_multiplier(family, r["k_full"],
                           f"Full-data fit; holdout train {r['k_train']:.2f}x/test {r['k_test']:.2f}x; exp={r['exp_term']:.2e}")
        else:
            set_multiplier(family, 1.0,
                           f"Unidentifiable: EXP(-S/W)={r['exp_term']:.1e}~0, spoilage term negligible, v1 correct (long shelf life)")
    ...
    print("\n=== HOLDOUT SUMMARY (4 families) ===")
    for family, r in report.items():
        if not r["identifiable"]:
            print(f"{family}: UNIDENTIFIABLE (exp={r['exp_term']:.1e}) -> pinned 1.0; "
                  f"sweep optimum z*_full={r['zstar_full']:+.2f}{'  [BOUNDARY]' if r['boundary'] else ''}")
            continue
        stable = abs(r["k_train"] - r["k_test"]) / r["k_train"] < 0.30
        print(f"{family}: multiplier train {r['k_train']:.2f}x / test {r['k_test']:.2f}x -> "
              f"{'STABLE (real)' if stable else 'MOVES (fudge-factor risk)'}; "
              f"held-out engine ${r['eng_test_total']:,.0f} vs test optimum ${r['te_min_total']:,.0f} "
              f"(+{r['gap_pct']:.1f}%); production full-data {r['k_full']:.2f}x"
              f"{'  [BOUNDARY - extend grid]' if r['boundary'] else ''}")
